    ...
    <div class="legendary-pokemon-section container">
      ...
      <div class="pokemon-carousel">
        ...
        <div class="carousel" ref="carouselRef">
          <div
            class="pokemon-card"
            v-for="pokemon in commonPokemons"
            :key="pokemon.name"
...
export default {
  ...
  setup() {
    const commonPokemons = ref([]);
    const featuredPokemon = ref(null);
    const loading = ref(true);
    const carouselRef = ref(null);

    const fetchCommonPokemons = async () => {
      try {
        ...
        commonPokemons.value = detailedPokemonResponses
          .map((res, index) => {
        ...
        featuredPokemon.value = commonPokemons.value[0];
      } catch (error) {
        console.error('Error fetching common Pokémon data:', error);
      } finally {
        loading.value = false;
      }
    };
